package utils;

public class SceneLocator {
    public static final String FIRST_PAGE="/views/first_page.fxml";
    public static final String SIGNUP_PAGE = "/views/create_client.fxml";
    public static final String LOGIN_PAGE="/views/login.fxml";
    public static final String CHANGE_PASSWORD_PAGE = "/views/changepassword.fxml";
    public static final String HOME_PAGE = "/views/homepage.fxml";
    public static final String SEE_PENALTIES="/views/penalties.fxml";
    public static final String SEE_PAYMENTS="/views/seePayments.fxml";
    public static final String SEE_RESERVATIONS="/views/seeReservations.fxml";
    public static final String SEE_CONTRACTS="/views/contracts.fxml";
    public static final String SEE_CAR_DETAILS= "/views/car_details.fxml";
    public static final String RESERVATION_FORM= "/views/reservation_form.fxml";
    public static final String CONTRACT_FORM = "/views/contractform.fxml";
    public static final String DOCUMENTS="/views/documents.fxml";
    public static final String PAYMENT_FORM = "/views/payment.fxml";
    public static final String ADD_PENALTY = "/views/add-penalty.fxml";
    public static final String MAINTENANCE_FORM = "/views/maintenance_form.fxml";
    public static final String SEE_ABOUT = "/views/aboutProgram.fxml";
    public static final String SEE_ABOUT_STAFF = "/views/aboutProgramStaff.fxml";
    public static final String UPDATE_PENALTY_REQUESTS= "/views/updatePenalties&SpecialRequests.fxml";
    public static final String REVIEW_FORM = "/views/reviews.fxml";
    public static final String SEE_OFFERS = "/views/offers.fxml";
    public static final String SEE_MAINTENANCE = "/views/seemaintenance.fxml";
    public static final String UPDATE_MAINTENANCE="/views/update-maintenance.fxml";
    public static final String ADD_OFFER="/views/add-offers.fxml";
    public static final String SEE_INSURANCE="/views/insurance.fxml";
}